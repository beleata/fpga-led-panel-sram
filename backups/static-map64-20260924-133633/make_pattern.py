...
Initial wiring hypothesis: DMD_Panel_Templates.h, pattern 3 (GKGD P5 1/8).
Each lane's first 64 serial columns address the lower eight-row strip;
the next 64 address the upper strip. RGB2 is 16 physical rows below RGB1.
"""
...
def coordinate(scan_row, channel, chip, lane):
    ...
    return serial_column % 64, scan_row + (8 if serial_column < 64 else 0) + 16 * lane
