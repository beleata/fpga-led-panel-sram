// The verified static scan, restarted only after a complete SRAM frame commit.
module panel_scan #(
    parameter SCAN = 8,
    parameter CHIPS = 8,
    parameter ROW_MODE = 0,
    ...
    parameter STATIC_HOLD = 1
) (
    input clk, enable, refresh,
    input [47:0] colors,
    output [9:0] word_address,
    output fetch_active, stable,
    output reg refresh_taken = 0,
    ...
    reg [7:0] tick_div = 0;
    reg [5:0] rgb = 0;
    assign {B2,G2,R2,B1,G1,R1} = rgb;
    assign word_address = count[13:4];
    assign fetch_active = state == 9 && configured;
    assign stable = state == 11 && visible_uploads == 2;
    wire [5:0] pixel_bits;
    genvar lane;
    generate for (lane=0; lane<6; lane=lane+1) begin: gray
        wire [7:0] level = colors[lane*8 +: 8];
        // Preserve the verified 0x0100 maximum; other levels use 0x0000..00FE.
        assign pixel_bits[lane] = count[3:0] == 7 ? level == 255 :
            (count[3] && level != 255 && level[7-count[2:0]]);
    end endgenerate
    ...
    always @(posedge clk) begin
        ...
        if (!enable) begin
            SCLK <= 0;
            tick_div <= 0;
        end else if (tick_div == HALF_TICKS - 1) begin
            tick_div <= 0;
            if (!SCLK && state != GAP) begin
                ...
                if (count == length - 1) begin
                    ...
                    if (state == 9) begin
                        state <= 11;
                        if (configured && visible_uploads < 2)
                            visible_uploads <= visible_uploads + 1'b1;
                    end else if (state == 11) begin
                        // Two visible uploads allow an intervening frame-start
                        // latch before holding the driver's stored image.
                        if (STATIC_HOLD && visible_uploads == 2 && refresh) begin
                            state <= GAP;
                            visible_uploads <= 0;
                            config_index <= 21;
                            refresh_taken <= 1;
                        end else if (STATIC_HOLD && visible_uploads == 2) state <= 11;
                        else state <= GAP;
                    end
