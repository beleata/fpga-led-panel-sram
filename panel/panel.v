...
`endif
    ...
    // One six-lane word per channel/chip pair; synchronous ROM uses block RAM.
    reg [5:0] pattern [0:1023];
    reg [5:0] pattern_rgb = 0;
    initial $readmemh("pattern.hex", pattern);
    always @(posedge clk) pattern_rgb <= pattern[count[13:4]];
            ...
            if (!SCLK && state != GAP) begin
                ...
                if (count == length - 1) begin
                    ...
                    if (state == 9) begin
                        ...
                        if (STATIC_HOLD && visible_uploads == 2) state <= 11;
                        else state <= GAP;
                    end
