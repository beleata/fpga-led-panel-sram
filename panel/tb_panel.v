...
module tb_panel;
    ...
    reg [21:0] seen = 0;
    reg [7:0] rows_seen = 0;
    reg [5:0] picture [0:1023];
    reg [15:0] lane_word [0:5];
    wire [5:0] lanes = {B2,G2,R2,B1,G1,R1};
    integer lane;
    reg [15:0] expected [0:21];
    initial begin
        $readmemh("pattern.hex", picture);
        for (lane=0; lane<6; lane=lane+1) lane_word[lane] = 0;
    ...
    always @(posedge SCLK) begin
        if (dut.state >= 3 && dut.state <= 7 && lanes !== {6{R1}})
            $fatal(1,"Configuration lanes differ");
        if (old_state != dut.state) begin
            ...
            if (old_state == 9) begin
                ...
                if (frames == 23) begin
                    ...
                    $display("PASS: register broadcast; full frames; all 8 binary addresses; all 6144 grayscale words of the static pattern");
                    $finish;
                end
            end
        ...
        if (dut.state == 9) begin
            ...
            rows_seen[{C,B,A}] = 1;
            if (!dut.configured && lanes != 0) $fatal(1,"Pixels before full initialization");
            for (lane=0; lane<6; lane=lane+1) begin
                lane_word[lane] = {lane_word[lane][14:0],lanes[lane]};
                if (dut.configured && clocks % 16 == 0 &&
                    lane_word[lane] !== (picture[(clocks-1)/16][lane] ? 16'h0100 : 16'h0000))
                    $fatal(1,"Wrong grayscale at word=%0d lane=%0d",(clocks-1)/16,lane);
            end
        end
    end
endmodule
