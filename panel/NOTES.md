...
## Current hardware image: SRAM video frame receiver

2026-09-24: `../video/video.bin` is now programmed and verified. The user
confirmed the UART-loaded SRAM image is correct and stable, without unwanted
white pixels or flicker. Two external SRAM buffers hold six 1024-byte planes;
CRC16 and per-byte write/readback checks gate a frame commit. The fixed UART
is 2 Mbaud. The host sent the first frame and received ACK in 0.323 seconds.
Bad CRC, partial-frame timeout, duplicate retry and reconnect tests all passed
on the real board without changing the displayed image. See
`../video/README_BG.md` and `../video/VERIFICATION.md` for protocol and evidence.
This original panel-only source and its preserved backup are unchanged.

## Previous hardware image: UART link diagnostic, same panel picture

After the stable panel milestone, the user requested serial video RAM, then
asked to first identify swapped UART wires and measure maximum link speed.
The preceding FPGA image was `../link_test/link_diag.bin`, which instantiates this
unchanged panel driver and adds UART autobaud/XOR echo. See
`../link_test/README_BG.md` for hashes, test results and current state.
D0/D1 were physically swapped; the user corrected them. OLIMEXINO firmware
was backed up twice and updated to `iceprog_link` with configurable baud.
Verified: 1 MiB round trip at 2 Mbaud, no wrong/missing bytes; another ten
independent retraining tests passed. Link is currently at 2 Mbaud on COM18.
The diagnostic image itself does not implement SRAM/video RAM.
The stable panel-only image below remains preserved as a recovery baseline.

## Confirmed stable baseline: uninterrupted static scan

USER CONFIRMATION, 2026-09-24: geometry and colors are correct; the display
is fully stable, with no flicker and no unwanted bright pixels. This confirms
the uninterrupted static scan test below. No new programming is needed.
The authoritative reusable Bulgarian protocol guide is
`../PANEL_PROTOCOL_BG.md`. Source, verified bitstream, documentation and logs
are preserved with their directory structure in
`../backups/stable-panel-20260924/` and `../stable-panel-20260924.zip`.
This is the preferred recovery baseline; the earlier map32 build flickers.

After confirming the geometry, the user marked five flickering white pixels
in the arrow and six unwanted yellow pixels immediately inside the upper
circle edge in the upside-down reference. The marked positions correspond
approximately to upright y=16 and y=24, both scan address 0. These are user
observations, not measured electrical waveforms.

The earlier map32 baseline repeats initialization/configuration, uploads, a
32768-clock display hold and a 200-us clock stop every approximately 32.1 ms.
Row address 0 is held throughout initialization. This suggests frame-boundary
disturbance as one possible cause; it does not exclude row timing, driver
register settings, or signal integrity.

ONE diagnostic version now sets `STATIC_HOLD=1`: after 22 initialization
uploads and two visible uploads, state 11 loops continuously. The second
visible upload allows an intervening frame-start latch. There are no further
frame resets, data latches, configuration writes, or clock pauses. The ROM,
brightness, panel clock and within-row OE/address timing are unchanged.
This is a static-image diagnostic, not a finished dynamic framebuffer driver.

Tests passed: `test_mapping.py`, `tb_panel.v`, and `tb_static_hold.v` (8192
uninterrupted scan clocks across repeated hold-counter wraps; equal dwell
and OE counts on every row; no latch/data/reinitialization). Route uses
447 logic cells and 2 RAM blocks; 50.85 MHz timing passes the 25 MHz target.
Bitstream SHA256:
`b78fb7d55cf977bcf4902dc009b04d8d9af7a3111f0af55a4674b78caf37d09f`.
Programming log: `logs/program-static-continuous.log`.
Pre-test flash backup: `backups/20260924-134756-flash-prefix.bin`.

All three observations have now been confirmed: the figure remains intact,
the arrow pixels do not flicker, and the unwanted yellow pixels are gone.
The exact internal source of the old artifacts remains unmeasured; the
successful fix is the uninterrupted hold, not a change of pixel mapping.

## Earlier baseline: correct mapping, residual flicker
